Accounts.registerLoginHandler(function (loginRequest) {
  let token = loginRequest['oneTimePass.OTPToken'];

  if (!token)
    return undefined; // don't handle

  let {user} = OnetimePass._lookupToken(token);
...
Accounts.onLogin((data) => {
  ...
  Meteor.users.update({_id, 'services.oneTimePassword.tokens.hashedToken': otpStampedToken}, {
    $set: {
